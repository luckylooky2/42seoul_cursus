...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   assign_redirection_flag.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: chanhyle <chanhyle@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/20 17:21:28 by chanhyle          #+#    #+#             */
/*   Updated: 2022/07/20 18:44:33 by chanhyle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (syn->input[i] == syn->input[i + 1] && syn->meaning[i + 1] == 1)
	{
		if (syn->input[i] == '<')
			data->check = '2';
		else
	...
	else if ((syn->input[i] == syn->input[i + 1]
			&& syn->meaning[i + 1] == 0)
...
static void	assign_flag(t_syntax *syn, int *i, t_data *data)
{
	...
	{
		(*i)++;
		assign_consecutive_flag(syn, i, data);
	}
	else
	{
...
void	assign_redirection_flag(t_syntax *syn, t_data *data)
{
	...
	while (syn->input[i])
	{
		...
		else if (syn->input[i] == ' ' && syn->meaning[i] == 1)
			data->rd_flag[i] = ' ';
		else
			data->rd_flag[i] = data->check;
		i++;
